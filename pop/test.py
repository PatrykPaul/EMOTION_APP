...
def pokaz_popup():
    ...
    add_bubble("Share how you feel with every message you send.", outgoing=False)

    # === Pasek emocji – obrazki z przewijaniem; klik = NATYCHMIAST WYŚLIJ ===
    ...
    def on_emotion_click(text_to_send):
        add_bubble(text_to_send, outgoing=True)  # ← od razu wysyłamy bąbel po prawej
    ...
    def _on_mousewheel(event):
        ...
        else:           delta = 1 if event.num == 5 else -1
...
tb.Label(root, text="(nagłówek skaluje się • emocje wysyłają od razu)", bootstyle=SECONDARY).pack(pady=10)
